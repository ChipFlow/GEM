...
**Remaining**: a CI job that runs `cargo test` in `crates/opensta-to-ir` after building OpenSTA via `scripts/build-opensta.sh`. Today the corpus test (along with the existing CLI/integration tests) only runs locally because the opensta-to-ir crate isn't in the root workspace and the CI test job is `cargo test --lib` only. Tracked as the next small step on the WS4 punch list.
