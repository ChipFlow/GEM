...
# dependencies = []
# ///
"""Compare Loom and CVC simulation outputs for MCU SoC validation.
...
Timing comparison (optional): When --loom-timing-vcd is provided, compares
...
Usage:
    python3 compare_simulation.py <loom.vcd> <cvc.vcd> [--skip-cycles N] [--config <config.json>] [--skip-bits 0-5]
    python3 compare_simulation.py <loom.vcd> <cvc.vcd> --loom-timing-vcd <timing.vcd> [...]
"""
...
def parse_loom_timing_transitions(
    path: Path,
    clock_period: int,
    width: int = 44,
    output_port_map: dict[str, int] | None = None,
) -> list[BitTransition]:
    """Parse Loom timing VCD into per-bit transitions with arrival offsets."""
    code_to_bit: dict[str, int] = {}
    in_header = True
...
def run_timing_comparison(
    loom_transitions: list[BitTransition],
    ...
    """Compare timing between Loom and CVC transitions, print report."""
    ...
    loom_grouped = group_by_cycle_bit(loom_transitions)
    cvc_grouped = group_by_cycle_bit(cvc_transitions)

    common_keys = set(loom_grouped.keys()) & set(cvc_grouped.keys())
    loom_only = set(loom_grouped.keys()) - set(cvc_grouped.keys())
    cvc_only = set(cvc_grouped.keys()) - set(loom_grouped.keys())

    print(f"\n{'='*60}")
    print("Timing Comparison Report")
    print(f"{'='*60}")
    print(f"  Loom transitions (filtered): {len(loom_grouped)}")
    print(f"  CVC transitions (filtered):  {len(cvc_grouped)}")
    print(f"  Common (cycle,bit) pairs:    {len(common_keys)}")
    print(f"  Loom-only transitions:       {len(loom_only)}")
    ...
    # Compute differences: (cycle, bit, cvc_arrival, loom_arrival, diff)
    diffs: list[tuple[int, int, int, int, int]] = []
    for key in sorted(common_keys):
        cycle, bit = key
        cvc_arr = cvc_grouped[key]
        loom_arr = loom_grouped[key]
        diff = loom_arr - cvc_arr
        diffs.append((cycle, bit, cvc_arr, loom_arr, diff))

    abs_diffs = [abs(d[4]) for d in diffs]

    print(f"\n  Arrival Time Differences (Loom - CVC):")
    ...
    print(f"    {'Cycle':>7} {'Bit':>4} {'CVC(ps)':>10} {'Loom(ps)':>10} {'Diff(ps)':>10} {'%clk':>6}")
    for cycle, bit, cvc_arr, loom_arr, diff in diffs_sorted[:10]:
        pct = 100.0 * abs(diff) / clock_period
        print(f"    {cycle:>7} {bit:>4} {cvc_arr:>10,} {loom_arr:>10,} {diff:>+10,} {pct:>5.1f}%")

    # Timing-critical analysis
    near_edge_threshold = 0.8 * clock_period
    timing_critical: list[tuple[int, int, int, int, str]] = []

    for cycle, bit, cvc_arr, loom_arr, diff in diffs:
        cvc_near = cvc_arr > near_edge_threshold
        loom_near = loom_arr > near_edge_threshold
        if cvc_near != loom_near:
            if loom_near and not cvc_near:
                reason = "Loom late (near next edge), CVC safe"
            else:
                reason = "CVC late (near next edge), Loom safe"
            timing_critical.append((cycle, bit, cvc_arr, loom_arr, reason))

    # Check for transitions captured in different cycles
    edge_crossings: list[tuple[int, int, int, int]] = []
    for cycle, bit, cvc_arr, loom_arr, diff in diffs:
        cvc_same = cvc_arr < clock_period
        loom_same = loom_arr < clock_period
        if cvc_same != loom_same:
            edge_crossings.append((cycle, bit, cvc_arr, loom_arr))
    ...
    if timing_critical:
        print(f"\n    {'Cycle':>7} {'Bit':>4} {'CVC(ps)':>10} {'Loom(ps)':>10} Issue")
        for cycle, bit, cvc_arr, loom_arr, reason in timing_critical[:20]:
            print(f"    {cycle:>7} {bit:>4} {cvc_arr:>10,} {loom_arr:>10,} "
                  f"{reason}")
    ...
    if edge_crossings:
        ...
        print(f"    {'Cycle':>7} {'Bit':>4} {'CVC(ps)':>10} {'Loom(ps)':>10}")
        for cycle, bit, cvc_arr, loom_arr in edge_crossings[:10]:
            print(f"    {cycle:>7} {bit:>4} {cvc_arr:>10,} {loom_arr:>10,}")
...
def main() -> None:
    ...
    loom_timing_vcd: Path | None = None
    ...
    while i < len(args):
        if args[i] == "--skip-cycles" and i + 1 < len(args):
            ...
            i += 2
        elif args[i] == "--loom-timing-vcd" and i + 1 < len(args):
            loom_timing_vcd = Path(args[i + 1])
            i += 2
    ...
    if loom_timing_vcd:
        ...
        print(f"\nParsing Loom timing VCD: {loom_timing_vcd}")
        loom_timing = parse_loom_timing_transitions(
            loom_timing_vcd, clock_period, output_port_map=output_port_map,
        )
        ...
        run_timing_comparison(
            loom_timing, cvc_timing, clock_period,
